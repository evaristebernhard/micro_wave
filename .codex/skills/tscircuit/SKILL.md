...
- For fixed-layer bus interconnects, use `<autoroutingphase autorouter="bus_lanes" connections={[...]} />`. Read [Bus-lanes routing](./BUS_LANES.md) for endpoint selectors, fanout handoffs, length matching, and unsupported cases. Keep width and skew settings on `<bus />`; preserve the phase output for later global routing.
